...
| `W27-S02` | planned | add durable integration runtime configuration over local and Postgres backends and remove ad hoc publisher selection from the worker path | `cargo test --workspace --all-targets --all-features`, `./scripts/check-slice.sh --wave W27-integration-runtime-and-http-publisher --slice W27-S02 --lane integration` |
| `W27-S03` | planned | add one bounded HTTP publisher adapter with explicit timeout and status-code failure semantics | `cargo test --workspace --all-targets --all-features`, `./scripts/check-slice.sh --wave W27-integration-runtime-and-http-publisher --slice W27-S03 --lane integration` |
| `W27-S04` | planned | prove durable integration runtime reload and HTTP publication through Postgres rehearsal and the full wave gate | `./scripts/rehearse-infra.sh --profile db`, `./scripts/check-wave.sh --wave W27-integration-runtime-and-http-publisher` |
