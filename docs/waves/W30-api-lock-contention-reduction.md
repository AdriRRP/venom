...
| `W30-S02` | in_progress | replace the global API mutex with a read-write lock and route read-only handlers through shared reads | `cargo test --workspace --all-targets --all-features` |
| `W30-S03` | planned | close the wave and run the full wave gate | `./scripts/check-wave.sh --wave W30-api-lock-contention-reduction` |
