...
| Context Factor Provenance | projection field | The truthful source and concrete identity behind each individual effective context trait after overlay precedence is applied, such as `production:true` from collection profile `context:corp-api-baseline` or `vpn-restricted:true` from tag `tag:api`. | forcing operators to infer per-factor origin from whole-profile labels |
