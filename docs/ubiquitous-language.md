...
| Collection-Scoped Context Action | process | One explicit operator action that applies one managed context profile across every managed member of one closed release collection. | client-side loops that assign context profile member by member |
