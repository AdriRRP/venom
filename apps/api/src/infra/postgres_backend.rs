...
pub struct InventoryDefinitionSourceWatermarks {
    ...
    pub collection_scan_schedules: u64,
}
...
impl PostgresReadSnapshotBase {
    ...
    pub fn new(
        inventory: Arc<ComponentInventory>,
        read_model: Arc<FindingReadModel>,
        sources: PostgresReadSnapshotSources,
    ) -> Self {
        Self {
            inventory,
            read_model,
...
impl PostgresStore {
    ...
    pub fn rebase_live_sources_from(&mut self, source: &Self) {
        self.observed_change_watermark
            .store(source.observed_change_watermark(), Ordering::Relaxed);
        self.ingestion = Arc::clone(&source.ingestion);
        self.governance = Arc::clone(&source.governance);
        self.read_model = Arc::clone(&source.read_model);
        ...
        self.order = Arc::clone(&source.order);
        self.pending_integration_events = Arc::clone(&source.pending_integration_events);
        self.pending_integration_source_cursor = source.pending_integration_source_cursor.clone();
        self.system_event_index_snapshot_cache =
            Arc::clone(&source.system_event_index_snapshot_cache);
        self.system_event_source_cursor = source.system_event_source_cursor.clone();
        self.command_statuses_snapshot_cache = Arc::clone(&source.command_statuses_snapshot_cache);
        self.command_status_source_cursor = source.command_status_source_cursor.clone();
    }
    ...
    pub async fn add_component_to_collection(
        ...
        if result.change == venom_domain::AddCollectionComponentChange::Added {
            ...
            *self.ingestion_mut().inventory_mut() = candidate_inventory;
            self.inventory_definition_source_watermarks
                .collection_memberships = self
                .inventory_definition_source_watermarks
                .collection_memberships
                .max(membership_cursor);
            self.refresh_read_snapshot_caches();
        }
        Ok(result)
    }
    ...
    pub async fn materialize_collection_source(
        ...
        if result.change == MaterializeCollectionSourceChange::Materialized {
            ...
            let mut membership_cursor = 0_u64;
    ...
    pub async fn configure_collection_scan_schedule(
        ...
        if result.change == ConfigureCollectionScanScheduleChange::Configured {
            ...
            *self.ingestion_mut().inventory_mut() = candidate_inventory;
            self.inventory_definition_source_watermarks
                .collection_scan_schedules = self
                .inventory_definition_source_watermarks
                .collection_scan_schedules
                .max(u64::try_from(updated_at_micros).map_err(|_| {
                    "postgres collection scan schedule updated_at out of range".to_owned()
                })?);
            self.refresh_read_snapshot_caches();
        }
        Ok(result)
    ...
    pub async fn accept_risk(
        ...
        if result.change == AcceptRiskChange::Accepted {
            ...
            self.governance_journal_high_watermark =
                self.load_governance_identity_source_watermark().await?;
            self.refresh_read_model_and_release_board_snapshot_caches();
    ...
    async fn upsert_collection_scan_schedules(
        ...
        rows.into_iter()
            .try_fold(0_u64, |cursor, updated_at_micros| {
                Ok(cursor.max(u64::try_from(updated_at_micros).map_err(|_| {
                    "postgres collection scan schedule updated_at out of range".to_owned()
                })?))
            })
    }
    ...
    async fn persist_due_collection_scans(
        ...
        let command_status_cursor = self
            ...
            .await?;
        let collection_scan_schedule = self
            .upsert_collection_scan_schedules(&mut transaction, schedule_rows)
            .await?;
        let system_event_cursor = self
            .insert_system_events_in_transaction(&mut transaction, system_events)
            .await?;
    ...
    fn apply_due_collection_scan_state(
        ...
        self.inventory_definition_source_watermarks
            ...
            .inventory_definition_source_watermarks
            .collection_scan_schedules
            .max(tail_cursors.collection_scan_schedule);
        self.system_event_source_cursor =
    ...
    async fn append_provider_report_journals_in_transaction(
        ...
        self.upsert_provider_report_head_in_transaction(transaction, report, provider_report_id)
            .await?;
        self.append_provider_report_head_journal_in_transaction(
            transaction,
            report,
            provider_report_id,
        )
    ...
    async fn append_provider_report_head_journal_in_transaction(
        &self,
        transaction: &mut sqlx::Transaction<'_, sqlx::Postgres>,
        report: &ProviderScanReport,
        provider_report_id: i64,
        ...
        sqlx::query(&format!(
            ...
            self.names.provider_report_head_journal
        ))
        .bind(report.component_key.as_ref())
        .bind(artifact_kind_name(report.artifact.kind))
        .bind(report.artifact.identity.as_ref())
        ...
        .execute(&mut **transaction)
        .await
        .map_err(|error| format!("postgres provider report head journal append failed: {error}"))?;
        Ok(())
    }
    ...
    async fn init_schema(&self) -> Result<(), String> {
        ...
        self.create_provider_report_head_journal_table().await?;
        ...
        self.create_collection_snapshot_journal_table().await?;
    ...
    async fn rebuild_provider_report_heads_from_journal(&self) -> Result<(), String> {
        sqlx::query(&format!("DELETE FROM {}", self.names.provider_report_heads))
            .execute(&self.pool)
            .await
            .map_err(|error| format!("postgres provider report head reset failed: {error}"))?;
        sqlx::query(&format!(
            concat!(
                "INSERT INTO {} ",
                "(component_key, artifact_kind, artifact_identity, provider_key, observed_at_micros, freshness, knowledge_revision, findings, provider_report_id) ",
                "SELECT DISTINCT ON (component_key, artifact_kind, artifact_identity) ",
                "component_key, artifact_kind, artifact_identity, provider_key, observed_at_micros, freshness, knowledge_revision, findings, provider_report_id ",
                "FROM {} ",
                "ORDER BY component_key, artifact_kind, artifact_identity, id DESC"
            ),
            self.names.provider_report_heads,
            self.names.provider_report_head_journal
        ))
        .execute(&self.pool)
        .await
        .map_err(|error| format!("postgres provider report head journal repair failed: {error}"))?;
        Ok(())
    }

    async fn backfill_provider_report_head_journal_from_heads(&self) -> Result<(), String> {
        sqlx::query(&format!(
            concat!(
                "INSERT INTO {} ",
                "(component_key, artifact_kind, artifact_identity, provider_key, observed_at_micros, freshness, knowledge_revision, findings, provider_report_id) ",
                "SELECT component_key, artifact_kind, artifact_identity, provider_key, observed_at_micros, freshness, knowledge_revision, findings, provider_report_id ",
                "FROM {}"
            ),
            self.names.provider_report_head_journal,
            self.names.provider_report_heads
        ...
        .await
        .map_err(|error| {
            format!("postgres provider report head journal backfill from heads failed: {error}")
        })?;
        Ok(())
    }
    ...
    async fn load_collection_snapshot_rows_from_journal(
        &self,
        collection_keys: Option<&[Box<str>]>,
        ...
        sqlx::query_as::<_, CollectionSnapshotRow>(&format!(
            concat!(
                "SELECT DISTINCT ON (collection_key) collection_key, name, context_profile_key, source_kind, mode, source_component_keys, membership_component_keys ",
                "FROM {} ",
                "WHERE NOT $1 OR collection_key = ANY($2) ",
                "ORDER BY collection_key, id DESC"
            ),
            self.names.collection_snapshot_journal
        ))
        .bind(filtered)
        .bind(&collection_keys)
        .fetch_all(&self.pool)
        .await
        .map_err(|error| format!("postgres collection snapshot journal load failed: {error}"))
    }

    async fn backfill_collection_snapshot_journal_from_heads(
        &self,
        collection_keys: Option<&[Box<str>]>,
    ) -> Result<(), String> {
        if let Some(keys) = collection_keys
        ...
        sqlx::query(&format!(
            concat!(
                "INSERT INTO {} ",
                "(collection_key, name, context_profile_key, source_kind, mode, source_component_keys, membership_component_keys) ",
                "SELECT collection_key, name, context_profile_key, source_kind, mode, source_component_keys, membership_component_keys ",
        ...
        .await
        .map_err(|error| {
            format!("postgres collection snapshot journal backfill from heads failed: {error}")
        })?;
        Ok(())
    }
    ...
    async fn create_collection_sources_table(&self) -> Result<(), String> {
        ...
        .execute(&self.pool)
        .await
        .map_err(|error| format!("postgres collection sources table alter failed: {error}"))?;
        Ok(())
    }
    ...
    async fn create_collection_scan_schedules_table(&self) -> Result<(), String> {
        ...
        .await
        .map_err(|error| {
            format!("postgres collection scan schedules add updated_at failed: {error}")
        })?;
        sqlx::query(&format!(
    ...
    async fn create_provider_report_head_journal_table(&self) -> Result<(), String> {
        sqlx::query(&format!(
            concat!(
                "CREATE TABLE IF NOT EXISTS {} (",
                "id BIGSERIAL PRIMARY KEY, ",
        ...
        .await
        .map_err(|error| {
            format!("postgres provider report head journal table create failed: {error}")
        })?;
        sqlx::query(&format!(
            concat!(
                "CREATE INDEX IF NOT EXISTS provider_report_head_journal_latest_idx ",
                "ON {} (component_key, artifact_kind, artifact_identity, id DESC)"
            ),
            self.names.provider_report_head_journal
        ))
        .execute(&self.pool)
        .await
        .map_err(|error| {
            format!("postgres provider report head journal index create failed: {error}")
        })?;
        Ok(())
    }
    ...
    async fn create_finding_governance_identity_journal_table(&self) -> Result<(), String> {
        sqlx::query(&format!(
            concat!(
                "CREATE TABLE IF NOT EXISTS {} (",
                "id BIGSERIAL PRIMARY KEY, ",
                ...
                "package_version TEXT NOT NULL, ",
                "package_purl TEXT NOT NULL DEFAULT '', ",
                "governance_journal_id BIGINT NOT NULL, ",
                "created_at TIMESTAMPTZ NOT NULL DEFAULT NOW()",
                ")"
            ),
        ...
        .await
        .map_err(|error| {
            format!("postgres finding governance identity journal table create failed: {error}")
        })?;
        Ok(())
    }
    ...
    async fn create_collection_snapshot_journal_table(&self) -> Result<(), String> {
        sqlx::query(&format!(
            concat!(
                "CREATE TABLE IF NOT EXISTS {} (",
                "id BIGSERIAL PRIMARY KEY, ",
            ...
            self.names.collection_snapshot_journal, self.names.collections
        ))
        .execute(&self.pool)
        .await
        .map_err(|error| {
            format!("postgres collection snapshot journal table create failed: {error}")
        })?;
        sqlx::query(&format!(
            concat!(
                "CREATE INDEX IF NOT EXISTS collection_snapshot_journal_latest_idx ",
                "ON {} (collection_key, id DESC)"
            ),
            self.names.collection_snapshot_journal
        ))
        .execute(&self.pool)
        .await
        .map_err(|error| {
            format!("postgres collection snapshot journal index create failed: {error}")
        })?;
        Ok(())
    }
    ...
    async fn rebuild(&mut self) -> Result<(), String> {
        ...
        self.governance_journal_high_watermark = self
            ...
            .max(self.load_governance_identity_source_watermark().await?);
    ...
    async fn refresh_collections_from_remote(&mut self, lane_mask: i32) -> Result<(), String> {
        ...
        backend
            .load_collection_subgraph_by_changed_keys(
                self.inventory_definition_source_watermarks,
                lane_mask,
            )
            .await?;
    ...
    async fn refresh_collection_scan_schedules_from_remote(&mut self) -> Result<(), String> {
        self.load_collection_scan_schedules_after(
            self.inventory_definition_source_watermarks
                .collection_scan_schedules,
        )
        .await?;
    ...
    async fn load_collection_scan_schedules_after(
        &mut self,
        after_micros: u64,
    ) -> Result<(), String> {
        let schedules = sqlx::query_as::<_, (String, i32, String, i64, Option<i64>, Option<i32>, i64)>(&format!(
        ...
        .bind(
            i64::try_from(after_micros)
                .map_err(|_| "postgres collection scan schedule cursor out of range".to_owned())?,
        )
        .fetch_all(&self.pool)
        .await
        .map_err(|error| format!("postgres collection scan schedule delta load failed: {error}"))?;
        for (
            collection_key,
            cadence_minutes,
            freshness,
    ...
    async fn load_latest_provider_report_head_rows_from_journal(
        &self,
    ) -> Result<Vec<ProviderReportHeadRow>, String> {
        sqlx::query_as::<_, ProviderReportHeadRow>(&format!(
            concat!(
                "SELECT DISTINCT ON (component_key, artifact_kind, artifact_identity) ",
                "provider_report_id, provider_key, component_key, artifact_kind, artifact_identity, ",
                "observed_at_micros, freshness, knowledge_revision, findings ",
                "FROM {} ",
                "ORDER BY component_key, artifact_kind, artifact_identity, id DESC"
            ),
        ...
        .fetch_all(&self.pool)
        .await
        .map_err(|error| format!("postgres provider report head journal load failed: {error}"))
    }
    ...
    async fn load_governance_journal_after(
        ...
        let rows = sqlx::query_as::<_, (i64, i64, String, String, String, String, String, String, String, String, Option<String>, Option<i64>)>(&format!(
            concat!(
                "WITH changed AS (",
                "SELECT component_key, artifact_kind, artifact_identity, vulnerability_id, ",
        ...
        {
            self.apply_governance_journal_row(
                (
                    id,
                    component_key,
                    artifact_kind,
            ...
            self.governance_journal_high_watermark = self.governance_journal_high_watermark.max(
                u64::try_from(journal_identity_id).map_err(|_| {
                    "postgres governance identity journal id out of range".to_owned()
                })?,
            );
        }
        Ok(())
    ...
    async fn load_governance_identity_source_watermark(&self) -> Result<u64, String> {
        let max_id = sqlx::query_scalar::<_, Option<i64>>(&format!(
            "SELECT MAX(id) FROM {}",
            self.names.finding_governance_identity_journal
        ))
        ...
        .await
        .map_err(|error| {
            format!("postgres governance identity journal watermark read failed: {error}")
        })?
        .unwrap_or_default();
        u64::try_from(max_id)
            .map_err(|_| "postgres governance identity journal watermark out of range".to_owned())
    }
    ...
    async fn append_governance_journal_entry_in_transaction(
        ...
        let journal_id = sqlx::query_scalar::<_, i64>(&format!(
            concat!(
                "INSERT INTO {} ",
        ...
        self.append_governance_identity_journal_in_transaction(tx, finding, journal_id)
            .await
    }
    ...
    async fn append_governance_journal_entries_in_transaction(
        ...
        let journal_ids = query
            .build_query_as::<(i64,)>()
            .fetch_all(&mut **tx)
            .await
            .map_err(|error| format!("postgres governance journal batch insert failed: {error}"))?;
        for ((journal_id,), finding) in journal_ids.into_iter().zip(findings.iter()) {
            cursor = cursor.max(
                self.append_governance_identity_journal_in_transaction(tx, finding, journal_id)
                    .await?,
            );
    ...
    async fn append_governance_identity_journal_in_transaction(
        ...
        tx: &mut Transaction<'_, Postgres>,
        finding: &FindingRef,
        governance_journal_id: i64,
    ) -> Result<u64, String> {
        sqlx::query_scalar::<_, i64>(&format!(
            concat!(
                "INSERT INTO {} ",
                "(component_key, artifact_kind, artifact_identity, vulnerability_id, package_name, package_version, package_purl, governance_journal_id) ",
                "VALUES ($1, $2, $3, $4, $5, $6, $7, $8) RETURNING id"
            ),
            self.names.finding_governance_identity_journal
        ))
        ...
        .bind(finding.package.version.as_ref())
        .bind(finding.package.purl.as_deref().unwrap_or(""))
        .bind(governance_journal_id)
        .fetch_one(&mut **tx)
        .await
        .map_err(|error| {
            format!("postgres governance identity journal insert failed: {error}")
        })
        .and_then(|id| {
            u64::try_from(id)
    ...
    async fn upsert_collection_snapshot_head_in_transaction(
        ...
        .bind(source_kind)
        .bind(mode)
        .bind(Json(source_component_keys.clone()))
        .bind(Json(membership_component_keys.clone()))
        .execute(&mut **tx)
        .await
        ...
        .execute(&mut **tx)
        .await
        .map_err(|error| format!("postgres collection snapshot journal append failed: {error}"))?;
        Ok(())
    }
...
impl PostgresReadSnapshotLoader {
    ...
    pub async fn load(
        ...
        let (read_model, read_model_source_watermark, governance_source_watermark) = self
            ...
            .await?;
        let (system_event_index, system_event_source_cursor) = self
            .load_system_events_for_changed_lanes(&base, lane_mask)
            .await?;
        let (command_statuses, command_status_source_cursor) = self
            .load_command_statuses_for_changed_lanes(&base, lane_mask)
            .await?;
    ...
    async fn load_inventory_for_changed_lanes(
        ...
        let inventory = if lane_mask & CHANGE_LANE_COLLECTION_SCHEDULES != 0 {
            self.load_collection_schedule_inventory_snapshot(
                inventory,
                base.inventory_definition_source_watermarks
                    .collection_scan_schedules,
            )
    ...
    async fn load_read_model_for_changed_lanes(
        &self,
        base: &PostgresReadSnapshotBase,
        inventory: Arc<ComponentInventory>,
        lane_mask: i32,
    ) -> Result<(Arc<FindingReadModel>, u64, u64), String> {
        if lane_mask & (CHANGE_LANE_READ_MODEL | CHANGE_LANE_GOVERNANCE) == 0 {
            return Ok((
                Arc::clone(&base.read_model),
        ...
        let read_model = self
            ...
            .await?;
        let (read_model_source_watermark, governance_source_watermark) = self
            .load_read_model_source_watermarks(
                base.read_model_source_watermark,
                base.governance_source_watermark,
                lane_mask,
            )
    ...
    async fn load_system_events_for_changed_lanes(
        &self,
        base: &PostgresReadSnapshotBase,
        lane_mask: i32,
    ) -> Result<(Arc<SystemEventQueryIndex>, EventSourceCursor), String> {
        if lane_mask & CHANGE_LANE_SYSTEM_EVENTS != 0 {
            self.load_system_event_index_snapshot(
                Arc::clone(&base.system_event_index),
                base.system_event_source_cursor.clone(),
            )
            .await
        } else {
            Ok((
    ...
    async fn load_command_statuses_for_changed_lanes(
        &self,
        base: &PostgresReadSnapshotBase,
        lane_mask: i32,
        ...
        if lane_mask & CHANGE_LANE_COMMAND_STATUSES != 0 {
            self.load_command_statuses_snapshot(
                Arc::clone(&base.command_statuses),
                base.command_status_source_cursor.clone(),
            )
            .await
        } else {
            Ok((
    ...
    async fn load_collection_schedule_inventory_snapshot(
        &self,
        inventory: Arc<ComponentInventory>,
        collection_scan_schedules_after_micros: u64,
    ) -> Result<Arc<ComponentInventory>, String> {
        let mut backend = PostgresStore::detached(self.pool.clone(), self.names.clone());
        backend.ingestion = Arc::new(FindingIngestion::from_inventory_arc(inventory));
        backend
            .load_collection_scan_schedules_after(collection_scan_schedules_after_micros)
            .await?;
        backend.refresh_inventory_snapshot_cache();
        Ok(backend.inventory_snapshot_arc())
    }
...
struct TableNames {
    ...
    provider_report_head_journal: Box<str>,
    ...
    collection_snapshot_journal: Box<str>,
...
impl TableNames {
    fn new(schema: &str) -> Result<Self, String> {
        ...
        Ok(Self {
            ...
            provider_report_head_journal: format!("{schema}.provider_report_head_journal")
                .into_boxed_str(),
            ...
            finding_governance_journal: format!("{schema}.finding_governance_journal")
                .into_boxed_str(),
            finding_governance_identity_journal: format!(
            ...
            collection_snapshot_heads: format!("{schema}.collection_snapshot_heads")
                .into_boxed_str(),
            collection_snapshot_journal: format!("{schema}.collection_snapshot_journal")
                .into_boxed_str(),
...
mod tests {
    ...
    async fn detached_postgres_read_snapshot_advances_read_model_source_watermark_for_new_reports()
    {
        ...
        let finding_journal_high_watermark = sqlx::query_scalar::<_, i64>(&format!(
        ...
        assert_eq!(
            refreshed_snapshot.read_model_source_watermark,
            u64::try_from(finding_journal_high_watermark)
                .expect("finding journal watermark should fit in u64")
        );
        assert_eq!(
    ...
    async fn postgres_reopen_allows_explicit_legacy_bootstrap_when_authoritative_snapshot_tables_are_empty()
     {
        ...
        let provider_report_journal_rows = sqlx::query_scalar::<_, i64>(&format!(
            "SELECT COUNT(*) FROM {}",
            reopened.names.provider_report_head_journal
        ))
        .fetch_one(&reopened.pool)
        .await
        .expect("provider report head journal count should load");
        let collection_snapshot_journal_rows = sqlx::query_scalar::<_, i64>(&format!(
            "SELECT COUNT(*) FROM {}",
            reopened.names.collection_snapshot_journal
        ))
        .fetch_one(&reopened.pool)
        .await
    ...
    #[tokio::test]
    async fn postgres_reopen_backfills_provider_report_head_journal_from_heads_when_missing() {
        let Some(database_url) = postgres_test_url() else {
            return;
        };
        let schema = temp_schema("provider_report_head_journal_backfill");
        let mut backend = PostgresStore::open(&database_url, &schema)
            .await
            .expect("postgres backend should open");
        register_payments_component(&mut backend).await;
        ...
        .with_knowledge_revision("fixture-db:2026-05-23");
        let _ = backend
            .record_scan_report(&report)
            .await
            .expect("provider report should persist");
    ...
    #[tokio::test]
    async fn postgres_reopen_backfills_collection_snapshot_journal_from_heads_when_missing() {
        let Some(database_url) = postgres_test_url() else {
            return;
        };
        let schema = temp_schema("collection_snapshot_journal_backfill");
        let mut backend = PostgresStore::open(&database_url, &schema)
            .await
            .expect("postgres backend should open");
        register_payments_component(&mut backend).await;
    ...
    #[tokio::test]
    async fn postgres_read_model_refresh_uses_identity_journal_cursors() {
        let Some(database_url) = postgres_test_url() else {
            return;
        };
        let schema = temp_schema("identity_journal_refresh");
        let mut backend = PostgresStore::open(&database_url, &schema)
            .await
            .expect("postgres backend should open");
        register_payments_component(&mut backend).await;
        ...
        let since_change_watermark = backend
            ...
            .await
            .expect("current watermark should be readable after first report");
        let base_report_cursor = backend.read_model_source_watermark();
        let base_governance_cursor = backend.governance_source_watermark();
        let snapshot_base = snapshot_base(&backend);
        ...
        assert!(refreshed.read_model_source_watermark > base_report_cursor);
        assert!(refreshed.governance_source_watermark > base_governance_cursor);
        assert_eq!(
            refreshed
                .read_model
                .active_finding_count("component:payments-api", &artifact()),
    ...
    #[tokio::test]
    async fn detached_postgres_collection_refresh_reloads_only_changed_collections() {
        let Some(database_url) = postgres_test_url() else {
            return;
        };
        let schema = temp_schema("collection_delta_refresh");
        let mut backend = PostgresStore::open(&database_url, &schema)
            .await
            .expect("postgres backend should open");
        register_release_collection_fixture(&mut backend).await;
        let _ = backend
            ...
            .await
            .expect("second collection should persist");
        let _ = backend
            .add_component_to_collection("release:2026.07", "component:customer-portal")
            .await
            .expect("second collection membership should persist");
        ...
        let since_change_watermark = backend
            .current_change_watermark()
            .await
            .expect("current watermark should be readable after collection setup");
        let snapshot_base = snapshot_base(&backend);
        ...
        let _ = writer
            ...
            .await
            .expect("updated collection source should persist");
        let _ = writer
            .configure_collection_scan_schedule(
                "release:2026.06",
                30,
        ...
        let refreshed = loader
            .load(since_change_watermark, snapshot_base)
            .await
            .expect("detached fresh read should load collection deltas");

        let changed_source = refreshed
            .inventory
            .collection_source("release:2026.06")
            .expect("changed collection source should reload");
        assert_eq!(changed_source.component_keys().len(), 2);
        assert_eq!(
            refreshed
                .inventory
                .collection_members("release:2026.07")
                .expect("unchanged collection members should remain visible"),
            vec![Box::<str>::from("component:customer-portal")]
        );
        assert!(
            refreshed
                .inventory
                .collection_scan_schedule("release:2026.06")
                .is_some()
        );
    }
    ...
    async fn postgres_live_refresh_reloads_system_events_incrementally() {
        ...
        assert!(
            follower
                ...
                .any(|kind| kind == SystemEventKind::ScanCommandEnqueued)
        );
    }
